...
function useColorProcessor(seriesType) {
  const colorProcessors = React40.useContext(ColorContext);
  if (!seriesType) {
...
function DefaultChartsLegend(props) {
  ...
  const labelStyle = React45.useMemo(
  ...
  const padding = React45.useMemo(() => getStandardizedPadding(paddingProps), [paddingProps]);
  const getItemSpace = React45.useCallback((label, inStyle = {}) => {
  ...
  const [seriesWithPosition, legendWidth, legendHeight] = React45.useMemo(() => {
    let x = 0;
    let y = 0;
...
//# sourceMappingURL=chunk-KB3JTFHM.js.map
